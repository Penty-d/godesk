...
`cmd/godesk/main.go` 是 CLI 入口。Cobra 命令注册、处理和无额外依赖的 TUI 位于 `internal/cli`。共享逻辑按职责拆分：`internal/config` 处理全局配置、扫描根目录、项目索引和 `.godesk.yaml`；`internal/project` 处理 Go 模块扫描和文件发现；`internal/envfile` 与 `internal/compose` 负责解析项目文件；`internal/docker`、`internal/logtail`、`internal/ports`、`internal/runner` 封装运行时集成。`up`、`ps` 和 `logs` 等 Docker Compose 命令使用 `internal/docker`。用户文档位于 `README.md` 和 `README.zh-CN.md`；开发文档位于 `docs/`。
