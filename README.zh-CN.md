...
Doctor 会检查项目根目录、`go.mod`、已配置的 env 文件、compose 文件、日志文件、健康检查 URL 格式、Docker CLI 和 `lsof`。
...
它会使用解析到的 `compose_file` 运行 Docker Compose，并打印 Docker 返回的服务表格。
